...
exports.getAllUnits = (req, res) => {
  ...
  exports.getAllUnitsBySession = async(req, res) => {
    let cid = req.params.cid;
     ...
     Group_Master.findAll({ where: {cid:cid} })
    .then(units=>{
      res.status(200).json({
...
exports.getUnitByName = async(req, res) => {
    ...
    try{
    const unit = await Group_Master.findOne({ where: {name:name} });
    if (!unit) {
...
exports.getUnitById = (req, res) => {
  ...
  Group_Master.findById(id, (err, unit) => {
    if (err) {
...
exports.updateUnitById = async(req, res) => {
 // const { id } = req.params.id;
  ...
  try {
    const item =await Group_Master.findOne({ where: {id:id} })
...
exports.deleteUnitById = async(req, res) => {
    ...
    try {
      ...
      const unit = Group_MasterDelete.build({
          name: name,
          user:user.Username,
          ip:ip,
          block:true,
